...
export default function FilterModal({ isOpen, onClose, dictionary }: FilterModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <main className="relative z-20 w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <header className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-30">
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900">{dictionary?.filters?.title || "Filters"}</h1>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto hide-scroll p-8 space-y-10">
          {/* Location */}
          <section>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{dictionary?.filters?.location || "Location"}</label>
            <div className="relative group">
              ...
              <input 
                ...
                placeholder={dictionary?.filters?.location_placeholder || "City, neighborhood, or address"} 
                type="text" 
                value={location}
          ...
          <section>
            <div className="flex justify-between items-end mb-4">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">{dictionary?.filters?.price_range || "Price Range"}</label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-background-light p-3 rounded-lg border border-transparent focus-within:border-mosque/30 transition-colors">
                <label className="block text-[10px] text-gray-500 uppercase font-medium mb-1">{dictionary?.filters?.min_price || "Min Price"}</label>
              ...
              <div className="bg-background-light p-3 rounded-lg border border-transparent focus-within:border-mosque/30 transition-colors">
                <label className="block text-[10px] text-gray-500 uppercase font-medium mb-1">{dictionary?.filters?.max_price || "Max Price"}</label>
                <div className="flex items-center">
                  <span className="text-gray-400 mr-1">$</span>
                  <input 
                    className="w-full bg-transparent border-0 p-0 text-gray-900 font-medium focus:ring-0 text-sm outline-none" 
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">{dictionary?.filters?.property_type || "Property Type"}</label>
              <div className="relative">
                ...
                >
                  <option value="Any Type">{dictionary?.filters?.property_types?.["Any Type"] || "Any Type"}</option>
                  <option value="House">{dictionary?.filters?.property_types?.["House"] || "House"}</option>
                  <option value="Apartment">{dictionary?.filters?.property_types?.["Apartment"] || "Apartment"}</option>
                  <option value="Condo">{dictionary?.filters?.property_types?.["Condo"] || "Condo"}</option>
                  <option value="Townhouse">{dictionary?.filters?.property_types?.["Townhouse"] || "Townhouse"}</option>
                  <option value="Villa">{dictionary?.filters?.property_types?.["Villa"] || "Villa"}</option>
                  <option value="Penthouse">{dictionary?.filters?.property_types?.["Penthouse"] || "Penthouse"}</option>
                </select>
          ...
          {/* Amenities */}
          <section>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">{dictionary?.filters?.amenities || "Amenities & Features"}</label>
                ...
                return (
                  <label key={amenity.id} className="cursor-pointer group relative">
                    ...
                    >
                      ...
                      {dictionary?.filters?.amenity_list?.[amenity.label] || amenity.label}
                    </div>
                    {isActive && (
        ...
        <footer className="bg-white border-t border-gray-100 px-8 py-6 sticky bottom-0 z-30 flex items-center justify-between">
          ...
          >
            {dictionary?.filters?.clear_all || "Clear all filters"}
          </button>
          <button 
            onClick={handleApply}
          ...
          >
            {dictionary?.filters?.show_homes || "Show Homes"}
